...
visit maze place =
  state
    ( \olds ->
        if place `elem` olds
          then ((), olds)
          else case lookup place maze of
            Nothing -> ((), place : olds)
            Just s -> ((), let tmp = place : olds in ff s tmp maze)
    )

ff :: [String] -> [String] -> [(String, [String])] -> [String]
ff [] tmp _ = tmp
ff (x : xs) tmp maze = let a = execState (visit maze x) tmp in ff xs a maze
...
allSums (x : xs) = do
  xt <- allSums xs
  if (x + xt) == xt then if x == xt then [xt] else [x, xt] else if x == (x + xt) then [x + xt, xt] else [x + xt, x, xt]
...
f1 k acc x = do if x + acc <= k then Just (x + acc) else Nothing
...
f2 acc x = state (\olds -> if x `elem` olds then (acc, olds) else (acc + x, x : olds))
...
instance Functor SL where
  -- implement fmap
  -- fmap :: (a-> m b) -> SL (Int -> (a,Int,[String])) -> SL (Int->(f a ,Int ,[String]))
  fmap f (SL g) = SL (\x -> let (a, n, s) = g x in (f a, n, s))
...
instance Monad SL where
  -- implement return and >>=
  return x = SL (\olds -> (x, olds, []))

  -- SL a -> (a -> SL b) -> SL b
  (>>=) f g = SL h
    where
      h olds =
        let (a, s1, s) = runSL f olds
            b1 = g a
         in runSL b1 s1