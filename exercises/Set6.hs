...
instance Ord Country where
  ...
  (<=) x y = x == y -- and me?
  min a b = if a <= b then a else b -- and me?
  max a b = if a <= b then b else a -- and me?
...
instance Eq RationalNumber where
  (RationalNumber px py) == (RationalNumber qx qy) = px * qy == py * qx
...
simplify (RationalNumber n d) = let cop = gcd n d in RationalNumber (n `div` cop) (d `div` cop)
...
instance Num RationalNumber where
  ...
  abs (RationalNumber qn qd) = RationalNumber (abs qn) (abs qd)
  signum (RationalNumber qn qd) = RationalNumber (signum qn * signum qd) 1
  fromInteger x = RationalNumber x 1
  negate (RationalNumber qn qd) = simplify (RationalNumber (negate qn) qd)
...
class Cycle a where
  ...
  stepMany :: Int -> a -> a
  stepMany 0 a = a
  stepMany n a = step $ stepMany (n -1) a
...
instance Cycle Color where
  ...
  stepMany 0 c = c
  stepMany 1 c = step c
  stepMany 2 c = step $ step c
  stepMany n c = stepMany (n `mod` 3) c
...
instance Cycle Suit where
  ...
  stepMany 1 c = step c
  stepMany 2 c = step $ step c
  stepMany 3 s = step $ step $ step s
  stepMany n s = stepMany (n `mod` 4) s
