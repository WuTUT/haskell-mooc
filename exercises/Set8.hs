...
paintSolid color (Shape shape) (Picture base) = Picture f
  where
    f c = if shape c then color else base c
...
paint (Picture pat) (Shape shape) (Picture base) = Picture f
  where
    f c = if shape c then pat c else base c
...
instance Transform Flip where
  apply FlipX (Picture p) = Picture f where f (Coord x y) = p (Coord (- x) y)
  apply FlipY (Picture p) = Picture f where f (Coord x y) = p (Coord x (- y))
  apply FlipXY (Picture p) = Picture f where f (Coord x y) = p (Coord y x)
...
instance Transform Blur where
  apply Blur (Picture p) = Picture f
    where
      f (Coord x y) =
        let (Color r0 g0 b0) = p (Coord x y)
            (Color r1 g1 b1) = p (Coord (x -1) y)
            (Color r2 g2 b2) = p (Coord x (y -1))
            (Color r3 g3 b3) = p (Coord (x + 1) y)
            (Color r4 g4 b4) = p (Coord x (y + 1))
         in Color ((r0 + r1 + r2 + r3 + r4) `div` 5) ((g0 + g1 + g2 + g3 + g4) `div` 5) ((b0 + b1 + b2 + b3 + b4) `div` 5)
