...
instance Functor List where
  fmap f Empty = Empty
  fmap f (LNode a lst) = LNode (f a) (fmap f lst)
...
instance Functor TwoList where
  ...
  fmap f (TwoNode a b lst) = TwoNode (f a) (f b) (fmap f lst)
...
inBoth x y = [a | a <- toList x, a `elem` y]
...
instance Foldable List where
  foldr f z Empty = z
  foldr f z (LNode a lst) = f a (foldr f z lst)
...
instance Foldable TwoList where
  ...
  foldr f z (TwoNode a b lst) = f a (f b (foldr f z lst))
