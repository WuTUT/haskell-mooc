...
totalPrice (MkShoppingEntry _ d i) = d * fromIntegral i
...
buyOneMore (MkShoppingEntry s d i) = MkShoppingEntry s d (i+1)
...
setName name p = MkPerson {name = name ,age = getAge p}

-- setAge does likewise for age
setAge :: Int -> Person -> Person
setAge age p = MkPerson {name = getName p ,age = age}
...
up p = Position {x = getX p , y = 1 + getY p}

-- right increases the x value of a position by one
right :: Position -> Position
right p = Position {x = 1 + getX p , y = getY p}
...
data UpDown = Up {v :: Int} | Down {v::Int}
...
rgb Green = [0,1.0,0]
rgb Blue = [0,0,1.0]
rgb (Mix c1 c2) = [ (x+y) / 2 | (x,y) <- zip (rgb c1) (rgb c2)]
...
toNat n
 | n == 0 = Just Zero
 | n < 0 = Nothing
 | otherwise = let Just a = toNat (n -1) in Just (PlusOne a )
