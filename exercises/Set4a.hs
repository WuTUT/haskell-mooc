...
middle x y z
  | x <= y = if y <= z then y else if z <= x then x else z
  | otherwise = if z <= y then y else if z <= x then z else x
...
longest ::(Ord a) => [[a]]->[a]
longest a =minimumBy (\x y ->compare (head x) (head y)) [x | x <- a,length x == maximum (map length a)]
