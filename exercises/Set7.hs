...
member a (Set []) = False
member a (Set (x : xs)) = (a == x) || member a (Set xs)
...
add a (Set (x : y : xs))
  | a < x = Set (a : x : y : xs)
  | a == x = Set (x : y : xs)
  | x < a && a < y = Set (x : a : y : xs)
  | a == y = Set (x : y : xs)
  | a > y = let Set lf = add a (Set (y : xs)) in Set (x : lf)
...
data State = Start | Error | Finished | S1 | S21 | S22 | S31 | S4
  deriving (Eq, Show)

step :: State -> Event -> State
step Finished _ = Finished
step Start AddEggs = S1
step S1 AddFlour = S21
step S21 AddSugar = S22
step S1 AddSugar = S31
step S31 AddFlour = S22
step S22 Mix = S4
step S4 Bake = Finished
step _ _ = Error
...
average (a :| []) = a
average (a :| xs) = (a + sum xs) / fromIntegral (1 + length xs)
...
reverseNonEmpty (a :| []) = a :| []
reverseNonEmpty (a :| xs) = let l = reverse (a : xs) in head l :| tail l
...
instance Ord a => Semigroup (Set a) where
  ...
  (<>) (Set []) (Set b) = Set b
  (<>) (Set (x : xs)) (Set (y : ys))
    | x == y = let Set s = Set xs <> Set ys in Set (x : s)
    | x < y = let Set s = Set xs <> Set (y : ys) in Set (x : s)
    | x > y = let Set s = Set (x : xs) <> Set ys in Set (y : s)
...
passwordAllowed s (ContainsSome cs) = foldr (\a b -> b || elem a cs) False s
passwordAllowed s (DoesNotContain cs) = foldr (\a b -> b && notElem a cs) True s
...
data Arithmetic = Value Integer | Expression Arithmetic String Arithmetic
  deriving (Show)
...
operation "+" a1 a2 = Expression a1 "+" a2
operation "*" a1 a2 = Expression a1 "*" a2
